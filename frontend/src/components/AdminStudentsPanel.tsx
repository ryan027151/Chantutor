  ...
  return (
    <label className="flex flex-col gap-1.5">
      ...
      <input
        ...
        className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-base text-slate-900 placeholder-slate-400 focus:outline-none focus:border-amber-500/60 focus:ring-1 focus:ring-amber-500/25 transition-colors"
      />
    </label>
  );
}
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <div className="w-64 shrink-0 flex flex-col border-r border-slate-200 bg-white">
        <div className="px-4 py-4 border-b border-slate-200">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-sm bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full font-medium">{students.length}</span>
          </div>
          <input
            type="search"
            ...
            className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-base text-slate-700 placeholder-slate-400 focus:outline-none focus:border-amber-500/40 focus:ring-1 focus:ring-amber-500/20 transition-colors"
          />
        </div>
        <div className="flex-1 overflow-y-auto">
          {loading ? (
            <p className="px-4 py-5 text-sm text-slate-400">Loading…</p>
          ) : filtered.length === 0 ? (
            <p className="px-4 py-5 text-sm text-slate-400">No students found.</p>
          ) : filtered.map(s => (
            <button
              key={s.id}
              type="button"
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0">
                  <p className="text-base font-semibold text-slate-900 truncate">{s.first_name} {s.last_name}</p>
                  <p className="text-sm text-slate-400 capitalize">{s.role}</p>
                </div>
              </div>
            </button>
          ))}
        ...
        {!selected ? (
          <div className="flex h-full items-center justify-center">
            <div className="text-center flex flex-col items-center gap-2">
              ...
              <p className="text-base text-slate-400">Select a student to view their profile</p>
            </div>
          </div>
        ) : (
          <div className="p-6 flex flex-col gap-5 max-w-4xl">
            ...
            <div className="bg-slate-50 rounded-xl border border-slate-200 p-5">
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-full bg-amber-500/10 border border-amber-500/20 flex items-center justify-center text-lg font-bold text-amber-500 shrink-0">
                  {selected.first_name[0]}{selected.last_name[0]}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg font-bold text-slate-900">{selected.first_name} {selected.last_name}</h3>
                  <span className="text-sm font-semibold text-amber-600 capitalize">{selected.role}</span>
                </div>
                <div className="flex gap-4 shrink-0 items-center">
                  ...
                  <div className="flex flex-col gap-1.5 ml-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 border border-slate-200 hover:border-slate-300 transition-colors"
                    >
                    ...
                    {!confirmDeleteStudent ? (
                      <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium text-slate-400 hover:text-red-500 hover:bg-red-50 border border-transparent hover:border-red-200 transition-colors"
                      >
                        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button
                          ...
                          className="px-2 py-1 rounded-lg text-sm text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors"
                        >
                          ✕
                        </button>
                      </div>
            ...
            {linkedParents.length > 0 && (
              <div className="bg-slate-50 rounded-xl border border-slate-200 px-5 py-4">
                <p className="text-sm font-bold uppercase tracking-widest text-slate-400 mb-3">Linked Parents</p>
                <div className="flex flex-col gap-2">
                  {linkedParents.map(p => (
                    <div key={p.id} className="flex items-center gap-3">
                      <div className="w-7 h-7 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-sm font-bold text-slate-500 shrink-0">
                        {p.first_name[0]}{p.last_name[0]}
                      </div>
                      <span className="text-base text-slate-700">{p.first_name} {p.last_name}</span>
                      <span className="ml-auto text-sm text-slate-400 capitalize">parent</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {/* ── Test history ── */}
            <div>
              <p className="text-sm font-bold uppercase tracking-widest text-slate-400 mb-3">Test & Practice History</p>
              {loadingTests ? (
                <p className="text-sm text-slate-400 py-4">Loading…</p>
              ) : tests.length === 0 ? (
                <div className="bg-slate-50 rounded-xl border border-slate-200 px-5 py-8 text-center text-base text-slate-400">
                ...
                <div className="flex flex-col gap-2.5">
                  {tests.map(test => {
                    ...
                    return (
                      <div key={test.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div className="flex items-center gap-3 px-4 py-3.5">
                          ...
                          >
                            <div className="flex items-center gap-2 min-w-0">
                              <svg className={`w-3.5 h-3.5 text-slate-400 shrink-0 transition-transform ${isExpanded ? "rotate-90" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                              </svg>
                              <p className="text-base font-semibold text-slate-900 truncate">{test.test_name || "Untitled"}</p>
                            </div>
                            <p className="text-sm text-slate-400 mt-0.5 ml-5">
                              ...
                              {pct !== null && <span className="text-amber-600 font-semibold ml-1.5">{pct}% correct</span>}
                            </p>
                          </button>
                          ...
                          {test.score !== null && (
                            <>
                              <button
                                ...
                                className="text-sm font-medium px-3 py-1.5 rounded-lg border border-slate-200 text-slate-600 hover:text-slate-900 hover:border-slate-300 hover:bg-slate-50 transition-colors shrink-0"
                              >
                                View Results
                              </button>
                              <button
                                ...
                                disabled={ai === "loading"}
                                className="text-sm font-medium px-3 py-1.5 rounded-lg border border-amber-200 text-amber-600 bg-amber-50 hover:bg-amber-100 transition-colors disabled:opacity-40 shrink-0"
                              >
                          ...
                          {isConfirming ? (
                            ...
                            <div className="flex gap-1 shrink-0">
                              <button
                                ...
                                className="px-2.5 py-1.5 rounded-lg text-sm font-medium text-slate-400 hover:text-amber-600 hover:bg-amber-50 border border-transparent hover:border-amber-200 transition-colors"
                              >
                                Reset
                              </button>
                              <button
                                ...
                                className="px-2.5 py-1.5 rounded-lg text-sm font-medium text-slate-400 hover:text-red-500 hover:bg-red-50 border border-transparent hover:border-red-200 transition-colors"
                              >
                                Delete
                              </button>
                            </div>
                        ...
                        {isExpanded && stats && (
                          <div className="border-t border-slate-200 px-5 py-4">
                            <p className="text-sm font-bold uppercase tracking-widest text-slate-400 mb-3">Question Breakdown</p>
                            ...
                            <div className="flex gap-3 text-sm text-slate-400">
                              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-emerald-400 inline-block" />{correct} correct</span>
                              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-red-400 inline-block" />{stats.filter(q => q.is_correct === false).length} incorrect</span>
                              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded bg-slate-300 inline-block" />{test.total_questions - stats.length} skipped</span>
                            </div>
                          </div>
                        )}
                        ...
                        {typeof ai === "object" && ai !== null && (
                          <div className="border-t border-slate-200 px-5 py-4 grid grid-cols-3 gap-3">
                        ...
                        {ai === "error" && (
                          <div className="border-t border-slate-200 px-5 py-3 text-sm text-red-500">
      ...
      {editingProfile && selected && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-white border border-slate-200 rounded-2xl shadow-2xl w-full max-w-sm flex flex-col">
            ...
            <div className="p-6 flex flex-col gap-4">
              ...
              <label className="flex flex-col gap-1.5">
                ...
                <select
                  ...
                  title="Role"
                  className="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 text-base text-slate-900 focus:outline-none focus:border-amber-500/60 transition-colors"
                >
              ...
              {profileError && (
                <p className="text-sm text-red-500 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{profileError}</p>
              )}
            </div>
            <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between">
              ...
              <button
                ...
                className="px-5 py-2 rounded-lg text-base font-bold bg-amber-500 hover:bg-amber-400 text-white transition-colors disabled:opacity-50"
              >
